...
class ApiCallMemoryConsumerTest {
    ...
    @BeforeEach
    void setUp() {
        consumer = new ApiCallMemoryConsumer(embeddingPort, memoryStore);
    }
    ...
    void successful_call_stored_as_PROCEDURAL_memory() throws Exception {
        ...
        var payload = buildPayload("GET", "/v1/data", 200, 45L, "SUCCESS");
    ...
    void server_error_stored_as_EPISODIC_memory() throws Exception {
        when(embeddingPort.embed(anyString())).thenReturn(FAKE_EMBEDDING);
        var payload = buildPayload("POST", "/v1/items", 500, 200L, "FAILURE");
    ...
    void client_error_stored_as_SEMANTIC_memory() throws Exception {
        when(embeddingPort.embed(anyString())).thenReturn(FAKE_EMBEDDING);
        var payload = buildPayload("GET", "/v1/missing", 404, 15L, "UNKNOWN");
    ...
    void embedding_failure_skips_storage_without_throwing() {
        ...
        var payload = buildPayload("GET", "/v1/data", 200, 10L, "SUCCESS");
    ...
    private String buildPayload(String method, String path, int code, long latencyMs, String outcome) {
        try {
